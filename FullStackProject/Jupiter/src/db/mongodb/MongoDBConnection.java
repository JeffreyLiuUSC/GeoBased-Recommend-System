...
public class MongoDBConnection implements DBConnection{
	...
	public List<Item> searchItems(double lat, double lon, String term) {
		System.out.println("Entering MongoDB searchItems phase 1");
		TicketMasterAPI tmAPI = new TicketMasterAPI();
		List<Item> items = tmAPI.search(lat, lon, term);
		System.out.println("Entering MongoDB searchItems phase 2, items " + items);
		for (Item item : items) {
			saveItem(item);
		}
		return items;
	...
	public void saveItem(Item item) {
		System.out.println("Entering MongoDB saveItem phase 1");
		FindIterable<Document> iterable = db.getCollection("items")
				.find(eq("item_id",item.getItemId()));
		System.out.println("Entering MongoDB searchItems phase 2, boolean is " + iterable.first());
		if (iterable.first() == null) {
